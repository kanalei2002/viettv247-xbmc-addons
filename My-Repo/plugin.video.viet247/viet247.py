...
addon = xbmcaddon.Addon('plugin.video.viet247')
profile = xbmc.translatePath(addon.getAddonInfo('profile'))
mysettings = xbmcaddon.Addon(id='plugin.video.viet247')
home = mysettings.getAddonInfo('path')
fanart = xbmc.translatePath(os.path.join(home, 'fanart.jpg'))
version = "1.1.4"
#datafile = xbmc.translatePath(os.path.join(home, 'data.json'))

url = 'http://www.viettv247.com/viettv/data.json.gz'
...
#namex is vpoptv, vtv1, vtv2, lifetv,.....
...
def add_dir_link (namex):
        name = data['directories'][namex]['desc']
...
def update_chn_list():
	...
	if (url_error == "no"):
		...
		if (data['version'] != r_data['version']):
			dialog = xbmcgui.Dialog()
			ack = dialog.yesno(addon.getLocalizedString(30005), addon.getLocalizedString(30006), addon.getLocalizedString(30007))
			if ack:
				d = open(datafile, 'r+')
				d.seek(0)
				d.write(ff)
				d.close()
				#dialog.ok(addon.getLocalizedString(30008), addon.getLocalizedString(30009))
...
def play_link(chn, src):
	...
	app = data['sources'][src]['app']
        ...
        if (src == 'viettv247' or src == 'globaltvnet' or src == 'vtcnew' or src == 'christian' or src =='vtoday'):
                full_url = videoUrl
        elif(src == 'vietfacetx' or src == 'vtmtv'):
                full_url = videoUrl+"/playlist.m3u8"
        elif(src == 'tv24x'):
                full_url = videoUrl+' swfVfy=1 live=1 timeout=20 token='+token+' playpath='+playpath+' flashVer='+flashVer+' pageUrl='+pageUrl+' tcUrl='+videoUrl+' swfUrl='+swfUrl
        else:
                full_url = videoUrl+' swfVfy=1 live=1 timeout=10 playpath='+playpath+' flashVer='+flashVer+' pageUrl='+pageUrl+' tcUrl='+videoUrl+' swfUrl='+swfUrl
        print 'url ' + full_url
